...
export default {
  ...
  resizeDocument: (root, args, context) => {
    if (!context.user) throw new Error('Sorry, you must be logged in to resize a document.');
    if (!Documents.findOne({ _id: args._id, owner: context.user._id }))
      throw new Error('Sorry, you need to be the owner of this document to resize it.');
    Documents.update(
      { _id: args._id },
      {
        $set: {
          ...args,
          resized: sanitizeHtml(args.resized),
          updatedAt: new Date().toISOString(),
        },
      },
    );
